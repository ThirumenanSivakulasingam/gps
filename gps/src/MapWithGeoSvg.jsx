...
const TOP_LEFT = [7.255520, 80.590514];       // [latN, lngW]
const BOTTOM_RIGHT = [7.251906, 80.593547];   // [latS, lngE]
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Make shapes clickable (narrow with a class selector if you want)
...
export default function MapWithGeoSvg() {
  ...
  // Node creation module state
  const [nodeCreationMode, setNodeCreationMode] = useState(false); // ← Enable/disable node creation
  const [customNodes, setCustomNodes] = useState({}); // ← Custom nodes created by user
  const [selectedNode, setSelectedNode] = useState(null); // ← Currently selected node for editing
  const [manualInput, setManualInput] = useState({ lat: '', lng: '', id: '' }); // ← Manual input form
  
  // Existing node editing state
  const [editableNodes, setEditableNodes] = useState({}); // ← Modified versions of existing nodes
  const [selectedExistingNode, setSelectedExistingNode] = useState(null); // ← Selected existing node
  const [editMode, setEditMode] = useState(false); // ← Enable editing of existing nodes
  ...
  const handleBuildingPick = (buildingId) => {
    ...
    if (!destNode) {
      alert(`No entrance defined for ${buildingId}`);
      return;
    }
    if (!userPos) {
  ...
  const handleMapClick = (e) => {
    if (!nodeCreationMode) return;
    
    const { lat, lng } = e.latlng;
    const nodeId = `CUSTOM_${Date.now()}`;
    const newNode = { lat, lng, id: nodeId };
    ...
    setManualInput({ lat: lat.toFixed(6), lng: lng.toFixed(6), id: nodeId });
  };
    ...
    <div style={{ position: "relative", height: "100vh", width: "100%" }}>
      ...
      >
        ...
        <LocationWatcher 
          ...
          staleMs={10000}
          reportRaw={(ll, acc) => {
            console.log('GPS Fix:', ll, 'Accuracy:', acc);
          }}
        />
